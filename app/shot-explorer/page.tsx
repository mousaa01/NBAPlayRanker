...
// app/shots-explorer/page.tsx
//
// Shots Explorer (Dataset2)
// ------------------------
// What this page is for:
// - This is the “raw data proof” page for Dataset2. One row = one shot event from play-by-play.
// - Coaches/markers can literally SEE the underlying shot events we later aggregate into heatmaps + shot plans.
//
// What it calls (and why):
// - /pbp/meta/options
//   -> Populates dropdowns (seasons, teams, shotTypes, zones) so the UI stays aligned with whatever’s in the parquet.
//
// - /pbp/shots/preview
//   -> Returns a SMALL slice of rows + column list for fast browsing (so the UI never freezes).
//
// - /pbp/shots.csv
//   -> Exports a BIGGER slice (up to N rows) for Excel/R/Python deep dives.
//
// Why “Preview + CSV export” instead of loading everything?
// - The parquet is huge. Pulling thousands/millions of rows into the browser would be slow and pointless.
// - Preview = quick sanity check. CSV = real analysis workflow.
//
// Small implementation notes:
// - I merge Dataset1 meta (teamNames) with Dataset2 meta (seasons/shotTypes/zones) so labels look professional.
// - “Opponent / ShotType / Zone” are optional filters: empty means “All”.
// - `limit` is ONLY for preview; CSV export uses a higher fixed limit (5,000) by design.
