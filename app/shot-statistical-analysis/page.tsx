// app/shot-statistical-analysis/page.tsx
"use client";

/**
 * Shot Statistical Analysis (Dataset2)
 * ----------------------------------
 * This page is a “results dashboard” for Dataset2 ML/stat analysis endpoint.
 *
 * What it shows (all returned by backend /pbp/ml/analysis or equivalent):
 *  - Dataset snapshot (rows, games, seasons, feature columns, target)
 *  - EDA (target distribution histogram + missing value counts)
 *  - Correlation heatmap (numeric feature correlations, optionally including target)
 *  - Target↔Feature correlation ranking (top |corr|)
 *  - Feature selection (correlation filter, SelectKBest, RFE)
 *  - Model selection (best RMSE from tuned Ridge / RF / GB)
 *
 * UX features:
 *  - LocalStorage persistence for preferences (splits, auto-refresh, filters)
 *  - Auto-refresh timer (refreshes cached backend results)
 *  - “Recompute” button (forces backend to recompute analysis; slower)
 *  - Lightweight in-page visualizations (mini histogram, correlation table shading)
 *
 * Safety/performance:
 *  - requestIdRef prevents stale async responses from overwriting newer state
 *  - heatmapMaxCols limits correlation table size for UI performance
 */
...
/** Correlation matrix structure from backend */
type CorrMatrix = { labels: string[]; matrix: number[][] };

/** Row for target-feature correlation summary */
type TargetCorrRow = { feature: string; corr: number; abs: number };

/**
 * Backend response schema (typed so TS catches UI/backend drift early).
 * Keep it fairly strict for the fields we display, but we can still tolerate extra backend fields.
 */
