...
//
// Shot Heatmap (Dataset2)
// -----------------------
// What this page is for:
// - This is the “visual evidence” page for Dataset2 shots.
// - It renders a real on-court heatmap image (base64 PNG) generated by the backend.
// - It’s meant to be coach-friendly: filters → render → screenshot/export.
//
// What it calls:
// - fetchMetaOptions()     -> Dataset1 meta (teamNames) so we can show "TOR — Raptors" labels.
// - fetchPbpMetaOptions()  -> Dataset2 meta (seasons, teams, shotTypes, zones) for dropdown options.
// - fetchShotHeatmap()     -> Backend heatmap generator that returns { image_base64, caption?, n_shots? }.
//
// Why localStorage persistence:
// - This page is typically used in “iterate + compare” mode.
// - Saving the last filter state means you can refresh, come back later, and still have the same setup.
//
// Why requestIdRef exists:
// - Auto-render + slow renders can cause “out-of-order” responses.
// - requestIdRef lets us ignore stale results (so the UI never shows the wrong image).
//
// Why autoRender is debounced (350ms):
// - Without debounce, typing or flipping filters would spam the backend.
// - Debounce keeps UX smooth and protects the API.
//
// Presets (ALL/RIM/CORNER_3/etc.):
// - These are “best effort” because naming differs across datasets.
// - findBestOption tries exact match first, then substring match.
